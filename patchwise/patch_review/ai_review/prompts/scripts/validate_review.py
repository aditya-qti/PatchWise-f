...
Usage:
  ...
  validate_review.py --block-file <tmp/patch_N_block.html> \
      ...
      [--rules-file <tmp/patch_N_rules.md>] \
...
# ---------------------------------------------------------------------------
# Per-check remediation registry
# ---------------------------------------------------------------------------
# Every check category emitted as a Violation(check=...) MUST have an entry
...
REMEDIATION: dict[str, dict[str, str]] = {
    ...
    "self_audit": {
        "fix": "Fix the block's self-audit line so the recorded patch hash "
               "matches the diff that was reviewed.",
        "ref": "core.md#Your procedure — 7 mandatory steps",
    },
    "conditional_sections": {
        "fix": "Add the missing conditional section header (DT / DT-driver / "
               "Hardware Engineering) the patch triggers; the rules brief lists "
    ...
    "anchor_targets": {
        "fix": "Make every banner finding-card link "
               "<a href=\"#patch-<N>-finding-<K>\"> resolve to an existing "
               "canonical per-commit card id.",
        "ref": "orchestrator-workflow.md#Step 6",
    },
    "banner_dedup": {
    ...
    "stat_chips": {
        "fix": "Update the verdict-banner stat-row chip counts to match the "
               "reconciled per-commit finding totals.",
        "ref": "orchestrator-workflow.md#Step 6",
    },
    "verdict_banner": {
        "fix": "Ensure the report contains exactly one verdict banner "
               "(class=\"verdict-banner\") with a verdict pill.",
    ...
    "render_violations": {
        "fix": "Same as render_format: text slots hold inline HTML only; relocate "
               "block-level HTML to siblings.",
        "ref": "html-template.md#Finding-card render-safe pattern",
    },
    "raw_html": {
        "fix": "Escape literal < > & in user-supplied/code text inside text "
               "nodes and attributes (&lt; &gt; &amp; &quot;).",
        "ref": "html-template.md#HTML Conformance Rules",
    },
    "block_fragment": {
    ...
    "block_index": {
        "fix": "Fix the commit-block ordering/index so blocks appear in patch "
               "order 1..T.",
        "ref": "orchestrator-workflow.md#Step 6",
    },
    "patch_number": {
        "fix": "Correct the patch number recorded in the block so it matches the "
               "reviewed patch's series position.",
        "ref": "orchestrator-workflow.md#Step 6",
    },
    # --- Build / test evidence ---
    "build_break_order": {
        "fix": "When the W=1 build failed for a patch, the build-break finding "
               "must be the FIRST finding-card in that block (and its banner "
               "card first). Reorder accordingly.",
    ...
    "build_artifact_validity": {
        ...
        "ref": "orchestrator-workflow.md#Step 2",
    },
    ...
    "refactor_coverage": {
        ...
        "ref": "code-logic.md#3c.4 Interaction Picture",
    },
    "future_risk_gate": {
    ...
    "safe_clearance_gate": {
        "fix": "Remove safe/no-action conclusions that were emitted as "
               "[BUG]/[CONCERN] findings; a cleared item is not a finding.",
        "ref": "gate-rules.md#THREE-GATE RULE — FINDING VALIDATION, MANDATORY, SEQUENTIAL, NON-BYPASSABLE",
    },
    "severity_crash_floor": {
    ...
    "severity_restore_floor": {
        ...
        "ref": "hardware-eng.md#3f.1 Device Power State Before Register Access",
    },
    ...
    "codebase_audit_required": {
        "fix": "Add the three 'codebase audit: entrypoints/callees/siblings' "
               "lines to the Code Logic Maps for patches with function-level "
               "code changes.",
        "ref": "orchestrator-workflow.md#Step 2",
    },
    "on_demand_reads_record": {
    ...
    # --- Prompt / rules / corpus artifacts (orchestrator setup) ---
    "prompt_artifact": {
    ...
    "rules_artifact": {
        "fix": "Ensure the per-patch rules brief (tmp/patch_N_rules.md) exists; "
               "re-run scripts/assemble_rules.py for this patch.",
        "ref": "orchestrator-workflow.md#Step 2",
    },
    "source_corpus_required": {
    ...
    "compatible_fallback": {
        "fix": "Make the child compatible's fallback shape consistent with the "
               "parent/wrapper binding (single const vs fallback array).",
        "ref": "dt-binding.md#3d.1 DT-Binding Schema (`.yaml`) Rules",
    },
    "binding_compatible_shape_source_aware": {
        "fix": "Reconcile the binding's compatible shape with the actual driver/"
               "DTS usage in the source tree.",
        "ref": "dt-binding.md#3d.1 DT-Binding Schema (`.yaml`) Rules",
    },
    "binding_parent_compatible_consistency_source_aware": {
    ...
    "fast_path_restore_proof": {
        "fix": "Prove the fast/alternate path restores OPP/performance/clock "
               "state after suspend or helper replacement, or raise a finding.",
        "ref": "hardware-eng.md#3f.1 Device Power State Before Register Access",
    },
    "resource_abstraction_bypass_source_aware": {
        ...
        "ref": "hardware-eng.md#3f.5 Interrupt and DMA Context Constraints",
    },
    "resource_helper_guard_source_aware": {
        "fix": "Show the resource helper's guard/precondition holds on the "
               "touched path in the source tree, or raise a finding.",
        "ref": "code-logic.md#Pointer-returning API call checklist",
    },
    "helper_equivalence_requires_source_proof": {
        "fix": "A helper-equivalence claim needs source proof that all side "
               "effects of the original are preserved; read the helper body or "
               "raise a finding.",
        "ref": "code-logic.md#3c.4 Interaction Picture",
    },
    "helper_replacement_postcondition_source_aware": {
...
# — the completeness test should prevent this).  Mirrors the original static
# four-ref repair prompt.
...
def check_binding_parent_compatible_consistency_source_aware(
    ...
    discuss that contract mismatch or file a finding.
    """
    if not patch_corpus or source_root is None:
        return []
    visible_all = "\n".join(
        f"{b.subject}\n{b.raw_html}" for b in report.blocks
    )
...
# Deterministic markers proving an assembled rules brief (kept in sync with
# scripts/validate_rules_brief.py).
_RULES_GENERATED_MARKER = "Generated by scripts/assemble_rules.py"
_RULES_REQUIRED_MARKERS = (
...
def check_rules_file(
    *,
    ...
    """Validate the per-patch rules brief the prompt instructs the agent to read.

    The prompt's first line is required to read `patch_<N>_rules.md` (enforced by
    check_prompt_file), but that only validates the pointer. This makes the
    target itself a required, retained artifact: a daemon-managed run leaves the
    assembled brief in <project_path>/tmp until cleanup, so block repair can
    re-open exactly the rules the reviewer followed, and a missing/incomplete
    brief fails validation instead of silently disappearing.
    """
    ...
    if not rules_file.exists():
        return [Violation(
            "rules_artifact",
            str(rules_file),
            f"per-patch rules brief patch_{patch_number}_rules.md is referenced "
            "by the prompt but does not exist in <project_path>/tmp",
        )]
    text = rules_file.read_text(encoding="utf-8", errors="replace")
    missing = [
        marker for marker in (_RULES_GENERATED_MARKER, *_RULES_REQUIRED_MARKERS)
    ...
    if missing:
        return [Violation(
            "rules_artifact",
            str(rules_file),
            "per-patch rules brief is present but not a complete assemble_rules.py "
            "brief; missing marker(s): " + ", ".join(missing),
        )]
    return []
...
def run_block(
    ...
    violations.extend(check_rules_file(
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="per-patch rules brief (patch_N_rules.md) for --block-file validation",
    )
    ap.add_argument(
        "--evidence-file",
        type=Path,
